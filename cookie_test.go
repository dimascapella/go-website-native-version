...
func GetCookie(rw http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		fmt.Fprint(rw, "No Cookies For You")
	} else {
		name := cookie.Value
		fmt.Fprintf(rw, "Cookies Name is %s", name)
	}
}
...
func TestSetCookie(t *testing.T) {
	request := httptest.NewRequest(http.MethodGet, "http://localhost:8080/?name=Dimas", nil)
